...
const uvec2 dim = uvec2(1024, 512);
layout(binding = 4, set = 0, scalar) buffer EnvMap
{
  uvec3 envmap[];
};

uint ravel(uvec2 pixel, uvec2 shape) {
  return shape.x * pixel.y + pixel.x;
}

vec3 pickcolor_texture(vec2 uv){
    vec2 coord_texel = clamp(uv * (dim - 1), vec2(0), vec2(dim-1));

    uvec3 rucorner = envmap[ravel(uvec2(ceil(coord_texel.x),ceil(coord_texel.y)), dim)];
    uvec3 rbcorner = envmap[ravel(uvec2(ceil(coord_texel.x),floor(coord_texel.y)), dim)];
    uvec3 lucorner = envmap[ravel(uvec2(floor(coord_texel.x),ceil(coord_texel.y)), dim)];
    uvec3 lbcorner = envmap[ravel(uvec2(floor(coord_texel.x), floor(coord_texel.y)), dim)];

    //WARN: Manque un filtrage bilinéaire entre les quatres coins du texel... et facteur scale;
    return mix(mix(lbcorner, rbcorner, uv[0]), mix(lucorner, rucorner, uv[0]), uv[1]);
}
...
vec3 skyColor(vec3 d)
{
  ...
  float phi = acos(-d.y/length(d));

  vec2 uv = vec2((theta + 3.14/2.0) / 3.14, (phi + 3.14) / (2*3.14) );

  return pickcolor_texture(uv);
}
...
void main()
{
  ...
  const uvec2 envmap_resolution = uvec2(1000,1000);
  ...
  uint linearIndex       = ravel(pixel, resolution);
